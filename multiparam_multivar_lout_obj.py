...
class Multiparam_multivar_df:
    ...
    def l_obj_var(self,inp_params:dict,variable:dict):
        #p = inp_params
        ...
        #return ind_pdis_obj(ckt_params,ip['lout']['value(uH)'],create_ind_family_df(ip['lout']['family'])) 
        return Inductor_pdis(ckt_params)
    
    def df_var(self,inp_params,variable:dict):
        l_obj = self.l_obj_var(inp_params,variable)
        return pd.DataFrame.from_dict(l_obj.summary,orient='index',columns=[variable['value']]).T
